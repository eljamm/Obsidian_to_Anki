...
class AnkiConnect:
    ...
    def requestPermission(self, origin, allowed):
        ...
        if allowed:
            ...
            msg.setText(
                '"{}" requests permission to use Anki through AnkiConnect. Do you want to give it access?'.format(
                    origin
                )
            )
            ...
            msg.setCheckBox(
                QCheckBox(
                    text='Ignore further requests from "{}"'.format(origin), parent=msg
                )
            )
            ...
            # if the origin isn't an empty string, the user clicks "No", and the ignore box is checked
                ...
                origin
                and pressedButton == QMessageBox.No
                and msg.checkBox().isChecked()
            ):
                config = aqt.mw.addonManager.getConfig(__name__)
    ...
    def getNumCardsReviewedToday(self):
        return self.database().scalar(
            ...
            (self.scheduler().dayCutoff - 86400) * 1000,
        )
    ...
    @util.api()
    def createModel(
        self, modelName, inOrderFields, cardTemplates, css=None, isCloze=False
    ):
    ...
    def modelFieldRename(self, modelName, oldFieldName, newFieldName):
        ...
        if oldFieldName not in fieldMap:
            raise Exception(
                "field was not found in {}: {}".format(modelName, oldFieldName)
            )
    ...
    def modelFieldReposition(self, modelName, fieldName, index):
        ...
        if fieldName not in fieldMap:
            raise Exception(
                "field was not found in {}: {}".format(modelName, fieldName)
            )
