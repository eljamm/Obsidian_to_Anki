...
test_file_path = "tests/test_outputs/{}/Obsidian/{}/{}.md".format(
...
def test_cards_front_back_tag_type(col: Collection):
    ...
    assert (
        note1.fields[0]
        == "The idea of {{c1::cloze paragraph style}} is to be able to recognise any paragraphs that contain {{c2::cloze deletions}}."
    )

    note2 = col.get_note(anki_IDs[1])
    assert (
        note2.fields[0]
        == "With {{c2::CurlyCloze}} enabled, you can also use the {{c1::easier cloze formatting}},<br />\nbut of course {{c3::Anki}}'s formatting is always an option."
    )
