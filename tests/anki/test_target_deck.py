...
test_file_paths = [
    [
        "tests/test_outputs/{}/Obsidian/{}/{}.md".format(
            test_name, test_name, test_name
        ),
        "MathematicsInNextLine",
    ],
    [
        "tests/test_outputs/{}/Obsidian/{}/{}.sameline.md".format(
            test_name, test_name, test_name
        ),
        "MathematicsInSameLine",
    ],
]
...
def test_cards_count(col: Collection):
    assert (
        len(
            col.find_cards(
                col.build_search_string(SearchNode(deck="MathematicsInNextLine"))
            )
        )
        == 1
    )
    assert (
        len(
            col.find_cards(
                col.build_search_string(SearchNode(deck="MathematicsInSameLine"))
            )
        )
        == 1
    )
