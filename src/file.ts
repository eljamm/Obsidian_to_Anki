    ...
    scanNotes() {
        for (let note_match of this.file.matchAll(this.data.NOTE_REGEXP)) {
            ...
            let parsed = new Note(
                ...
                this.data,
                this.data.add_context
                    ? this.getContextAtIndex(note_match.index)
                    : "",
            );
        ...
        for (let search_id of [true, false]) {
            for (let search_tags of [true, false]) {
                ...
                let regexp: RegExp = new RegExp(
                    regexp_str + tag_str + id_str,
                    "gm",
                );
                for (let match of findignore(
                    regexp,
                    this.file,
                    this.ignore_spans,
                )) {
                    ...
                    const parsed: AnkiConnectNoteAndID = new RegexNote(
                        ...
                        this.data,
                        this.data.add_context
                            ? this.getContextAtIndex(match.index)
                            : "",
                    );
                    if (search_id) {
                        if (
                            !this.data.EXISTING_IDS.includes(parsed.identifier)
                        ) {
                            if (parsed.identifier == CLOZE_ERROR) {
                        ...
                        parsed.note.tags.push(
                            ...this.global_tags.split(TAG_SEP),
                        );
                        this.regex_notes_to_add.push(parsed.note);
                        this.regex_id_indexes.push(
                            match.index + match[0].length,
                        );
                    }
                }
    ...
    writeIDs() {
        ...
        this.inline_id_indexes.forEach((id_position: number, index: number) => {
            const identifier: number | null =
                this.note_ids[index + this.notes_to_add.length]; //Since regular then inline
            if (identifier) {
                inline_inserts.push([
                    id_position,
        ...
        this.regex_id_indexes.forEach((id_position: number, index: number) => {
            const identifier: number | null =
                this.note_ids[
                    index +
                        this.notes_to_add.length +
                        this.inline_notes_to_add.length
