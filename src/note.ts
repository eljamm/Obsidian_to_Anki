    ...
    parse(
        ...
        if (url) {
            this.formatter.format_note_with_url(
                template,
                url,
                file_link_fields[this.note_type],
            );
        }
        ...
        if (data.add_obs_tags) {
            for (let key in template["fields"]) {
                for (let match of template["fields"][key].matchAll(
                    OBS_TAG_REGEXP,
                )) {
...
export class Note extends AbstractNote {
    ...
    getTags(): string[] {
        if (
            this.split_text[this.split_text.length - 1].startsWith(TAG_PREFIX)
        ) {
