...
const defaultDescs = {
    "Scan Directory":
        "The directory to scan. Leave empty to scan the entire vault",
...
export class SettingsTab extends PluginSettingTab {
        ...
        let link_field = new Setting(row_cells[2] as HTMLElement).addDropdown(
            async (dropdown) => {
                if (!plugin.fields_dict[note_type]) {
                    ...
                    if (
                        Object.keys(plugin.fields_dict).length !=
                        plugin.note_types.length
                    ) {
                        new Notice(
                        ...
                        try {
                            plugin.fields_dict =
                                await plugin.generateFieldsDict();
                            new Notice(
                                "Fields dictionary successfully generated!",
                            );
                        } catch (e) {
                            new Notice(
    ...
    setup_syntax() {
        ...
        for (let key of Object.keys(plugin.settings["Syntax"])) {
            new Setting(syntax_settings).setName(key).addText((text) =>
                text
                    .setValue(plugin.settings["Syntax"][key])
                    .onChange((value) => {
    ...
    setup_defaults() {
        let { containerEl } = this;
        const plugin = (this as any).plugin;
        let defaults_settings = containerEl.createEl("h3", {
            text: "Defaults",
        });
        ...
        if (
            !plugin.settings["Defaults"].hasOwnProperty("Scheduling Interval")
        ) {
        ...
        if (
            ...
            plugin.settings["Defaults"]["CurlyCloze - Highlights to Clozes"] =
                false;
        }
    ...
    setup_folder_deck(folder: TFolder, row_cells: HTMLCollection) {
        ...
        let folder_deck = new Setting(row_cells[1] as HTMLElement).addText(
            (text) =>
    ...
    setup_folder_tag(folder: TFolder, row_cells: HTMLCollection) {
        ...
        let folder_tag = new Setting(row_cells[2] as HTMLElement).addText(
            (text) =>
        ...
        new Setting(action_buttons)
            ...
            .addButton((button) => {
                button
                    ...
                    .onClick(async () => {
                        new Notice(
                            "Need to connect to Anki to update note types...",
                        );
                        try {
                            plugin.note_types =
    ...
    setup_ignore_files() {
        let { containerEl } = this;
        const plugin = (this as any).plugin;
        let ignored_files_settings = containerEl.createEl("h3", {
            text: "Ignored File Settings",
        });
        plugin.settings["IGNORED_FILE_GLOBS"] = plugin.settings.hasOwnProperty(
            "IGNORED_FILE_GLOBS",
        )
        ...
        descriptionFragment.createEl("span", {
            text: " for more information, examples and further resources.",
        });
        ...
        new Setting(ignored_files_settings)
            ...
            .addTextArea((text) => {
                text.setValue(plugin.settings.IGNORED_FILE_GLOBS.join("\n"))
                    .setPlaceholder(
                        "Examples: '**/*.excalidraw.md', 'Templates/**'",
                    )
                    .onChange((value) => {
