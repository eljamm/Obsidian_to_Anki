...
export default class MyPlugin extends Plugin {
    ...
    async getDefaultSettings(): Promise<PluginSettings> {
        ...
        this.note_types = (await AnkiConnect.invoke(
            "modelNames",
        )) as Array<string>;
    ...
    async scanVault(file: TFile) {
        ...
        try {
            ...
            new Notice(
                "Error, couldn't connect to Anki! Check console for error message.",
            );
            return;
        }
        new Notice(
        ...
        const data: ParsedSettings = await settingToData(
            this.app,
            this.settings,
            this.fields_dict,
        );
        ...
        if (file !== undefined) {
            ...
            if (scanDir !== null) {
                ...
                if (scanDir instanceof TFolder) {
                    console.info(
                        "Using custom scan directory: " + scanDir.path,
                    );
                    markdownFiles = this.getAllTFilesInFolder(scanDir);
                } else {
                    new Notice(
                        "Error: incorrect path for scan directory " +
                            this.settings.Defaults["Scan Directory"],
                    );
                    return;
                }
